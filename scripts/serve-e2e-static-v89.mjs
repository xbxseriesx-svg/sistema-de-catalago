...
function safePublicPath(pathname) {
  ...
  const candidate = resolve(PUBLIC_ROOT, relative);
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/__e2e_health') {
    return json(res, 200, { ok: true, service: 'asteryon-e2e-static' });
  }

  // Todos os /api devem ser interceptados pelo Playwright. Se algum escapar,
  // falhamos explicitamente em vez de devolver index.html e mascarar o teste.
  ...
  // O editor é SPA; rotas como /admin usam o mesmo index público.
  const index = resolve(PUBLIC_ROOT, 'index.html');
...
server.listen(PORT, HOST, () => {
  console.log(`[E2E static] http://${HOST}:${PORT} servindo ${PUBLIC_ROOT}`);
});
