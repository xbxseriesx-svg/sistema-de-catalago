...
const [html, core, capture, runtime, guard, versionText] = await Promise.all([
...
const release = Number(versionText.trim());
assert.ok(Number.isInteger(release) && release >= 91, 'Release lógica precisa preservar as camadas físicas V91.');
...
for (const file of ['preview-editor-v91-core.js', 'preview-editor-v91-capture.js', 'preview-editor-v91-runtime.js', 'preview-editor-v91-guard.js']) {
  assert.match(html, new RegExp(`${file.replaceAll('.', '\\.')}\\?v=${release}`), `${file} precisa estar no HTML publicado com cache key da release atual.`);
...
assert.doesNotMatch(html, /\?v=89(?:[&"'])/, 'HTML publicado não pode regredir para cache key V89.');
...
console.log(`QA V91 físico na release V${release}: Preview Final preenchido = Editor inicial editável, logos vinculados, cores e trava do Grupo 3: OK`);
