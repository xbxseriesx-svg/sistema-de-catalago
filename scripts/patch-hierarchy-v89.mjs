...
function replaceOnce(from, to, label) {
  ...
  if (first < 0) throw new Error(`Patch V89 hierarquia: trecho não encontrado nem já corrigido (${label}).`);
...
changed = replaceOnce(
  ...
  ':await Be.createHierarchy({level:a,name:o.trim(),parentId:a==="departamento"?null:n,sortOrder:100})',
  'parentId de Departamento',
) || changed;
...
const requirements = [
  ...
  ['botão Novo departamento', 'children:"Novo departamento"'],
  ['pai oculto no Departamento', '!d&&a!=="departamento"&&l.jsxs'],
  ['placeholder Departamento', 'a==="departamento"?"Ex.: Food Service"'],
  ['ações no Departamento', 'l.jsx(QF,{node:w,count:w.sections.length,onEdit:S,onDelete:b})'],
];
for (const [label, token] of requirements) {
  if (!source.includes(token)) throw new Error(`Patch V89 hierarquia incompleto: ${label}.`);
}
...
if (changed) {
  ...
  console.log('Patch V89 de hierarquia materializado/completado semanticamente.');
} else {
  console.log('Patch V89 de hierarquia já estava completo; validação sem alterações.');
}
