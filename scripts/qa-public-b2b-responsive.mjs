...
for (const token of [
  ...
  assert.ok(css.includes(token), `Contrato B2B público ausente: ${token}`);
}
...
for (const forbidden of [
  ...
  assert.equal(forbidden.test(css), false, `CSS público V96 contém compensação de zoom proibida: ${forbidden}`);
}

// Não permitir que o stylesheet público invada explicitamente o editor/admin.
assert.equal(css.includes('data-asteryon-surface="editor"'), false, 'V96 público não pode estilizar o editor.');
assert.equal(css.includes('/admin'), false, 'V96 público não pode conter regra administrativa.');

// 100% deve ser mais contido que a prévia histórica de 1440px/72px de hero.
assert.ok(css.includes('min-height: 440px'), 'Hero público deve ter enquadramento B2B contido em 100%.');
assert.ok(css.includes('font-size: clamp(36px, 3.6vw, 56px)'), 'Título do hero precisa ter escala visual confortável em desktop 100%.');
assert.ok(css.includes('padding: 48px var(--asteryon-public-inline)'), 'Hero precisa usar gutters B2B centrais em desktop.');
