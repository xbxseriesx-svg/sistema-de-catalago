...
export async function passwordRecord(password:string,saltHex?:string,iterations=210000){const salt=saltHex?Uint8Array.from(saltHex.match(/.{1,2}/g)!.map(x=>parseInt(x,16))):crypto.getRandomValues(new Uint8Array(16));const key=await crypto.subtle.importKey('raw',new TextEncoder().encode(password),'PBKDF2',false,['deriveBits']);const bits=await crypto.subtle.deriveBits({name:'PBKDF2',hash:'SHA-256',salt,iterations},key,256);const hash=[...new Uint8Array(bits)].map(b=>b.toString(16).padStart(2,'0')).join('');const saltOut=[...salt].map(b=>b.toString(16).padStart(2,'0')).join('');return {hash,salt:saltOut,iterations}}
export async function verifyPassword(password:string,hash:string,salt:string,iterations:number){const r=await passwordRecord(password,salt,iterations);if(r.hash.length!==hash.length)return false;let diff=0;for(let i=0;i<hash.length;i++)diff|=hash.charCodeAt(i)^r.hash.charCodeAt(i);return diff===0}
