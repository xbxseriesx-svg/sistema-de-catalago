...
export default {
  async fetch(req:Request,env:Env):Promise<Response>{
    try{
      ...
      if(path==='/api/health')return securityHeaders(ok({service:'sistema-de-catalago',database:'D1',r2:false,editor:'2.1'}));
      if(path.startsWith('/api/')){
        const originError=requireSameOrigin(req);if(originError)return securityHeaders(originError);
        await ensureBase(env);
        const handlers=[authRoute,pageRoute,templateRoute,catalogRoute];
