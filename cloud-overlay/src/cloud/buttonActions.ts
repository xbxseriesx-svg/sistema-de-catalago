...
export function resolveNodeAction(node:EditorNode):ResolvedNodeAction{
  ...
  if(explicit!=='none'){
    if(explicit==='product-info')return productId||explicitValue?base(node,'product-info',explicitValue||productId,false):base(node,'scroll','#catalogo',true);
    return base(node,explicit,explicitValue,false);
  }
  ...
  if(has(label,'whatsapp')){
    const configured=String(node.props?.whatsapp||node.props?.phone||'').trim();
    return configured?base(node,'whatsapp',configured,true):base(node,'scroll','#contato',true);
  }
  if(has(label,'telefone','ligar')){
    const configured=String(node.props?.phone||'').trim();
    return configured?base(node,'phone',configured,true):base(node,'scroll','#contato',true);
  }
  if(has(label,'email','e-mail')){
    const configured=String(node.props?.email||'').trim();
    return configured?base(node,'email',configured,true):base(node,'scroll','#contato',true);
  }
...
function scrollToAnchor(value:string){
  ...
  if(!id)return false;
  const target=document.getElementById(id)||document.querySelector(`[data-catalog-anchor="${id}"]`);
...
export function executeNodeAction(node:EditorNode):boolean{
  ...
  if(type==='url'){
    ...
    if(target==='new')window.open(value,'_blank','noopener,noreferrer');else window.location.assign(value);return true;
  }
  if(type==='whatsapp'){
    const digits=value.replace(/\D/g,'');if(!digits)return false;window.open(`https://wa.me/${digits}${message?`?text=${encodeURIComponent(message)}`:''}`,'_blank','noopener,noreferrer');return true;
  }
  if(type==='phone'){
  ...
  if(type==='product-info'){
    const productId=value||String(node.props?.productId||'').trim();if(!productId)return scrollToAnchor('#catalogo');
    window.dispatchEvent(new CustomEvent('asteryon:product-info',{detail:{productId}}));
    if(window.location.pathname==='/admin'||window.location.pathname.startsWith('/admin/'))void previewProductInfo(productId);
    return true;
  }
  return false;
}
