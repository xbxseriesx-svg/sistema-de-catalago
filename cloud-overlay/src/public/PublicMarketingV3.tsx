...
export function PublicMarketingV3({ config }: { config: MarketingConfig }) {
  ...
  return <div style={{ background: theme.background, color: theme.text }}>
    {banner.active && banner.mediaUrl && <section className="relative overflow-hidden" style={{ background: theme.surface }}>
      ...
      {(banner.title || banner.subtitle || banner.link) && <div className="absolute inset-0 flex items-center justify-center bg-black/25 p-6 text-center text-white"><div>
        ...
        {banner.link && <a href={safeLink(banner.link)} className="mt-5 inline-block rounded-lg px-5 py-2.5 text-sm font-black text-white shadow" style={{ background: theme.primary }}>Saiba mais</a>}
      </div></div>}
    </section>}
    ...
    {carousel.active && carousel.items.length > 0 && <Carousel config={carousel} theme={theme}/>}
  </div>;
}

function Carousel({ config, theme }: { config: MarketingConfig['carousel']; theme: MarketingConfig['theme'] }) {
  const [index, setIndex] = useState(0);
  useEffect(() => {
    if (!config.autoplay || config.items.length < 2) return;
    const timer = window.setInterval(() => setIndex(current => config.loop ? (current + 1) % config.items.length : Math.min(current + 1, config.items.length - 1)), Math.max(1200, 5000 / config.speed));
    return () => window.clearInterval(timer);
  }, [config.autoplay, config.loop, config.speed, config.items.length]);
  useEffect(() => { if (index >= config.items.length) setIndex(0); }, [index, config.items.length]);
  ...
  return <section className="relative overflow-hidden" style={{ background: theme.surface }}>
    ...
    {config.items.length > 1 && <div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 gap-1">{config.items.map((_, i) => <button key={i} aria-label={`Slide ${i + 1}`} onClick={() => setIndex(i)} className="h-2.5 w-2.5 rounded-full border border-white/70" style={{ background: i === index ? theme.primary : 'rgba(255,255,255,.45)' }}/>)}</div>}
  </section>;
}
