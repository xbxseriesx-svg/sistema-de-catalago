...
export const defaultDesignSystem: DesignSystem = {
  ...
  buttonRadius: 10,
  borderColor: '#DCE4EC',
  shadow: '0 12px 32px rgba(12, 32, 56, .10)',
  spacingUnit: 8,
}

export const defaultIdentity: IdentityConfig = {
  companyName: 'ASTERYON',
  logo: '',
  mobileLogo: '',
  favicon: '',
  pwaIcon: '',
...
const baseStyle = (): BlockStyle => ({
  ...
  hoverScale: 1,
  transitionMs: 180,
  desktop: { width: '100%', columns: 1, gap: 16, paddingTop: 0, paddingRight: 0, paddingBottom: 0, paddingLeft: 0 },
  tablet: { width: '100%', columns: 1, gap: 14 },
  mobile: { width: '100%', columns: 1, gap: 12 },
})
...
export const makeBlock = (type: BlockType, name?: string, parentId: string | null = null): BuilderBlock => {
  ...
  const defaults: Partial<Record<BlockType, { name: string; props: Record<string, unknown>; style?: Partial<BlockStyle> }>> = {
    ...
    header: { name: 'Cabeçalho', props: { showSearch: true, showNav: true, sticky: true }, style: { background: '#0B5D3B', color: '#FFFFFF' } },
    hero: { name: 'Banner Hero', props: { eyebrow: 'CATÁLOGO DIGITAL', title: 'Uma vitrine criada do seu jeito', subtitle: 'Personalize cada detalhe e publique somente quando estiver pronto.', ctaLabel: 'Ver promoções', ctaLink: '#promocoes', image: '' }, style: { background: '#0B5D3B', color: '#FFFFFF', borderRadius: 26 } },
    banner: { name: 'Banner Interno', props: { title: 'Banner promocional', subtitle: 'Mensagem de campanha', image: '', ctaLabel: 'Saiba mais', ctaLink: '#' }, style: { background: '#123A63', color: '#FFFFFF', borderRadius: 22 } },
    carousel: { name: 'Carrossel', props: { autoplay: true, interval: 5000, slides: ['Campanha 1', 'Campanha 2', 'Campanha 3'] } },
    card: { name: 'Card', props: { title: 'Card personalizado', text: 'Descrição do card', image: '', link: '#' } },
...
hero.order = 1
const promo = makeBlock('promotion')
promo.order = 2
promo.style.desktop.columns = 4
promo.style.tablet.columns = 2
promo.style.mobile.columns = 1
...
export const defaultDocument: EditorDocument = {
  ...
  name: 'Página Inicial',
  status: 'draft',
  blocks: [header, hero, promo, showcase, footer],
  designSystem: clone(defaultDesignSystem),
...
export const blockLibrary: Array<{ type: BlockType; label: string; group: string; description: string }> = [
  ...
  { type: 'header', label: 'Header', group: 'Estrutura', description: 'Cabeçalho com logo, busca e navegação.' },
