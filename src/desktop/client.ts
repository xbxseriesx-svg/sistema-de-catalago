...
export async function requestDesktopAiProposal(prompt: string, document: EditorDocument, researchRequested: boolean, selectedBlockId?: string | null) {
  ...
  return api<DesktopRemoteProposal>('/api/ai/proposal', { method:'POST', body:JSON.stringify({ prompt, researchRequested, context:{ pageId:document.pageId,pageName:document.name,theme:document.theme,designSystem:document.designSystem,blockTypes:document.blocks.map(block=>block.type),selectedBlock:selected?{type:selected.type,name:selected.name,props:selected.props,style:selected.style}:null } }) })
}
...
export async function generateDesktopAiImage(prompt: string, size: '1024x1024' | '1024x1536' | '1536x1024' | 'auto' = '1536x1024', quality: 'low' | 'medium' | 'high' | 'auto' = 'medium') {
  return api<{url:string;fileName:string;prompt:string}>('/api/ai/image',{method:'POST',body:JSON.stringify({prompt,size,quality})})
}
