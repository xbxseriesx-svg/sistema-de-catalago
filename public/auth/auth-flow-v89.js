(() => {
  ...
  async function consumeEmailSession() {
    ...
    if (params.error || params.errorCode) {
      ...
      throw new Error(expired
        ...
        : (params.errorDescription || 'O link de autenticação não pôde ser validado.'));
    }
  ...
  async function sessionStatus() {
    return api('/api/auth/account/session', { method: 'GET', headers: {} });
  }
  ...
  async function initConfirm() {
    ...
    try {
      ...
      if (state.authenticated) {
        ...
        if (passwordLink && !['invite', 'recovery'].includes(consumed.type || '')) {
          passwordLink.textContent = 'Definir ou alterar senha';
        }
        return;
      }
      if (!consumed.consumed) {
        showStatus('Não há uma sessão de confirmação ativa. Se o link expirou, solicite uma recuperação de senha.', 'error');
      }
    } catch (error) {
  ...
  async function initReset() {
    try {
      await consumeEmailSession();
    } catch (error) {
      showStatus(error.message || 'O link de recuperação não pôde ser validado.', 'error');
    }
  ...
  recoveryForm?.addEventListener('submit', async event => {
    ...
    try {
      const result = await api('/api/auth/account/recovery', {
        method: 'POST',
        body: JSON.stringify({ email }),
      });
      showStatus(result.message || 'Se a conta existir, o e-mail de recuperação será enviado.', 'success');
    } catch (error) {
      showStatus(error.message || 'Não foi possível solicitar a recuperação.', 'error');
    } finally {
  ...
  passwordForm?.addEventListener('submit', async event => {
    ...
    if (password.length < 10) {
      showStatus('A nova senha precisa ter pelo menos 10 caracteres.', 'error');
      return;
    }
    if (password !== confirmation) {
      showStatus('As duas senhas informadas não são iguais.', 'error');
      return;
    }

    setLoading(passwordForm, true);
    try {
      const result = await api('/api/auth/account/password', {
        method: 'PUT',
