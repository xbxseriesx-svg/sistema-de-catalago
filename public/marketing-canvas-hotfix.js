(() => {
  ...
  const applyLayout = () => {
    ...
    Object.assign(element.style, {
      ...
      zIndex: String(layout.zIndex), display: layout.visible && active(config) ? 'block' : 'none',
    });
  };
  ...
  const showSlide = index => {
    ...
    if (banner.active && banner.mediaUrl) {
      ...
      media.src = banner.mediaUrl; media.style.cssText = 'width:100%;height:100%;object-fit:cover;display:block';
      if (media instanceof HTMLVideoElement) { media.autoplay = banner.autoplay !== false; media.loop = banner.loop !== false; media.muted = banner.muted !== false; media.playsInline = true; }
      viewport.append(media);
    } else if (video.active && video.mediaUrl) {
      const media = document.createElement('video');
      media.src = video.mediaUrl; media.autoplay = video.autoplay !== false; media.loop = video.loop !== false; media.muted = video.muted !== false; media.controls = Boolean(video.controls); media.playsInline = true;
      media.style.cssText = 'width:100%;height:100%;object-fit:cover;display:block'; viewport.append(media);
    } else if (slides.length) {
      const slide = slides[index % slides.length];
      const media = document.createElement('img'); media.src = slide.url; media.alt = slide.alt || '';
      media.style.cssText = 'width:100%;height:100%;object-fit:cover;display:block'; viewport.append(media);
    }
  };
  ...
  const drag = (event, direction) => {
    ...
    const move = pointer => {
      const dx = (pointer.clientX - startX) / Math.max(.1, zoom), dy = (pointer.clientY - startY) / Math.max(.1, zoom);
  ...
  const button = (text, title, action) => {
    const item = document.createElement('button'); item.type = 'button'; item.textContent = text; item.title = title;
    ...
    item.addEventListener('pointerdown', event => event.stopPropagation()); item.addEventListener('click', event => { event.stopPropagation(); action(); });
    return item;
  };

  const mount = async () => {
    if (element || location.pathname !== '/admin') return;
    const target = canvas();
    if (!target) return;
    try { config = (await api('/api/public/marketing')).marketing; } catch { return; }
    ...
    element = document.createElement('section'); element.setAttribute(MARKER, 'true');
    ...
    const toolbar = document.createElement('div'); toolbar.setAttribute('data-marketing-drag-handle', 'true');
    ...
    const label = document.createElement('span'); label.textContent = 'Marketing · arraste para mover'; label.style.flex = '1'; toolbar.append(label);
    ...
    toolbar.append(button('Excluir', 'Excluir marketing', async () => {
      ...
      await save(); await Promise.allSettled(ids.map(id => api(`/api/admin/media/${encodeURIComponent(id)}`, { method: 'DELETE' })));
      element.remove(); element = null;
    }));
    toolbar.addEventListener('pointerdown', event => drag(event, 'move'));
    const viewport = document.createElement('div'); viewport.setAttribute('data-marketing-viewport', 'true'); viewport.style.cssText = 'position:absolute;inset:32px 0 0;overflow:hidden';
    element.append(toolbar, viewport);
    for (const direction of ['n','s','e','w','ne','nw','se','sw']) {
      const handle = document.createElement('div'); handle.setAttribute('data-marketing-resize', direction);
      ...
      handle.addEventListener('pointerdown', event => drag(event, direction)); element.append(handle);
    }
    target.append(element); applyLayout(); showSlide(0);
    let index = 0; clearInterval(interval); interval = setInterval(() => { if (config?.carousel?.autoplay !== false) showSlide(++index); }, Math.max(2000, 5000 / Math.max(.25, Number(config.carousel?.speed) || 1)));
  };

  const observer = new MutationObserver(() => mount()); observer.observe(document.documentElement, { childList: true, subtree: true });
  window.addEventListener('asteryon:marketing-live', event => { if (event.detail) { config = { ...event.detail, layout: config?.layout || event.detail.layout }; applyLayout(); showSlide(0); } });
  window.addEventListener('asteryon:marketing-updated', event => { if (event.detail) { config = { ...event.detail, layout: config?.layout || event.detail.layout }; applyLayout(); showSlide(0); } });
  mount();
})();
