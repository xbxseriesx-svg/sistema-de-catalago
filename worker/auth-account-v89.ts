...
function sessionResponse(data: Record<string, unknown>, accessToken: string, refreshToken: string, expiresIn: number) {
  const headers = new Headers({ 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
  headers.append('set-cookie', `${ACCESS_COOKIE}=${encodeURIComponent(accessToken)}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=${Math.max(60, expiresIn || 3600)}`);
  if (refreshToken) {
    headers.append('set-cookie', `${REFRESH_COOKIE}=${encodeURIComponent(refreshToken)}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=2592000`);
  }
  return new Response(JSON.stringify({ ok: true, ...data }), { status: 200, headers });
}

async function authUser(env: AuthAccountEnv, accessToken: string) {
...
async function acceptSession(req: Request, env: AuthAccountEnv) {
...
async function accountStatus(req: Request, env: AuthAccountEnv) {
...
async function requestRecovery(req: Request, env: AuthAccountEnv) {
  if (!sameOrigin(req)) return fail('Origem não autorizada', 403, 'ORIGIN');
  const input = await requestBody(req);
  ...
  const response = await fetch(`${env.SUPABASE_URL}/auth/v1/recover?redirect_to=${encodeURIComponent(redirectTo)}`, {
    method: 'POST',
    headers: {
      apikey: env.SUPABASE_PUBLISHABLE_KEY,
  ...
  // Resposta propositalmente genérica para não revelar se o e-mail existe.
  return ok({ message: 'Se existir uma conta para este e-mail, enviaremos um link para redefinir a senha.' });
}

async function updatePassword(req: Request, env: AuthAccountEnv) {
  if (!sameOrigin(req)) return fail('Origem não autorizada', 403, 'ORIGIN');
  const accessToken = cookie(req, ACCESS_COOKIE);
  if (!accessToken) return fail('Abra primeiro o link de confirmação ou recuperação enviado por e-mail', 401, 'RECOVERY_SESSION_REQUIRED');

  const input = await requestBody(req);
  const password = clean(input.password);
  if (password.length < 10) return fail('A nova senha precisa ter pelo menos 10 caracteres', 400, 'WEAK_PASSWORD');
  ...
  if (!response.ok) {
    ...
    return fail('Não foi possível atualizar a senha. Solicite um novo link se o problema persistir.', response.status === 401 ? 401 : 400, 'PASSWORD_UPDATE_FAILED');
  }

  return ok({ message: 'Senha definida com sucesso.' });
}

export async function handleAccountAuth(req: Request, env: AuthAccountEnv): Promise<Response | null> {
