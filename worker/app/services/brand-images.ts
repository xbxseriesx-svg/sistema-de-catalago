...
import { clean, fail, ok, secure } from '../http';
...
const USER_AGENT = 'ASTERYON-Catalog (Cloudflare Worker; brand image search)';
...
type SearchImage = {
  ...
  width?: number;
  height?: number;
  provider: 'google' | 'duckduckgo' | 'wikimedia';
};
...
async function getBrand(env: Env, brandId: string): Promise<BrandRow | null> {
  const rows = await adminFetch(
    env,
    `/rest/v1/brands?company_id=eq.${encodeURIComponent(COMPANY_ID)}&id=eq.${encodeURIComponent(brandId)}&select=id,name,slug,logo_url,data&limit=1`,
  );
...
async function signedResults(items: SearchImage[], env: Env) {
  ...
  for (const item of items.slice(0, 24)) {
    try {
      ...
      images.push({ ...item, url: normalized, signature: await signRemoteUrl(normalized, env) });
...
async function googleImageSearch(env: Env, query: string): Promise<SearchImage[]> {
  ...
  if (!key || !cx) throw new Error('Google Imagens não configurado');
...
async function duckduckgoImageSearch(query: string): Promise<SearchImage[]> {
  ...
  const vqd = extractVqd(await landing.text());
...
async function wikimediaImageSearch(query: string): Promise<SearchImage[]> {
  ...
  if (!response.ok) throw new Error(`Wikimedia indisponível (${response.status})`);
  const payload = await response.json() as any;
...
async function broadWebSearch(brand: BrandRow, env: Env) {
  ...
  return { images: await signedResults(dedupe(collected), env), notes };
}
...
async function handleSearch(req: Request, env: Env) {
  ...
  const providerRequest = clean(url.searchParams.get('provider')).toLowerCase();
  if (providerRequest === 'google') {
    if (!clean(env.GOOGLE_CSE_API_KEY) || !clean(env.GOOGLE_CSE_CX)) {
      return fail(
        'Google Imagens não está configurado neste Worker.',
        503,
        'GOOGLE_IMAGES_NOT_CONFIGURED',
      );
    }
    try {
      const images = await signedResults(dedupe(await googleImageSearch(env, `logo ${brand.name}`)), env);
      return ok({
        brand: { id: brand.id, name: brand.name, logoUrl: brand.logo_url || null },
        query: `logo ${brand.name}`,
        provider: 'google',
        providerMessage: `${images.length} resultado(s) encontrados no Google Imagens.`,
        images,
      });
    } catch (error) {
      return fail(
  ...
  if (clean(env.GOOGLE_CSE_API_KEY) && clean(env.GOOGLE_CSE_CX)) {
    try {
      const googleImages = await signedResults(dedupe(await googleImageSearch(env, `${brand.name} logo`)), env);
  ...
  if (web.images.length) {
    return ok({
      ...
      query: `${brand.name} logo`,
      provider: 'duckduckgo',
      providerMessage: `${web.images.length} resultado(s) encontrados na pesquisa ampla da web.`,
      images: web.images,
    });
  }

  try {
    const wikimedia = await signedResults(dedupe(await wikimediaImageSearch(`${brand.name} logo`)), env);
    if (wikimedia.length) {
      return ok({
        brand: { id: brand.id, name: brand.name, logoUrl: brand.logo_url || null },
        query: `${brand.name} logo`,
        provider: 'wikimedia',
        providerMessage: [...web.notes, `${wikimedia.length} resultado(s) encontrados no Wikimedia Commons.`].join(' '),
        images: wikimedia,
      });
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Wikimedia indisponível.';
    if (!web.notes.includes(message)) web.notes.push(message);
  }
  ...
  return ok({
    ...
    query: `${brand.name} logo`,
    provider: 'none',
    providerMessage: web.notes.join(' ') || 'Nenhuma imagem compatível encontrada.',
    images: [],
  });
}
...
async function fetchRemoteImage(req: Request, env: Env) {
  ...
  if (!sameSignature(signature, expected)) {
    return fail('Assinatura da imagem inválida', 403, 'INVALID_IMAGE_SIGNATURE');
  }
  ...
  for (let redirect = 0; redirect < 4; redirect++) {
    ...
    try {
      ...
      return fail(
        error instanceof Error ? error.message : 'Destino remoto bloqueado',
        403,
  ...
  if (!response || !response.ok) {
    return fail(`Falha ao baixar imagem${response ? ` (${response.status})` : ''}`, 502, 'IMAGE_DOWNLOAD_FAILED');
  }
  const mime = normalizeMime(response.headers.get('content-type'));
  if (!ALLOWED_MIMES.has(mime)) {
    return fail('Formato remoto não permitido. Use JPG, JPEG, PNG ou WEBP.', 415, 'IMAGE_FORMAT_UNSUPPORTED');
  }
  const announced = Number(response.headers.get('content-length') || 0);
  if (announced > MAX_REMOTE_BYTES) return fail('Imagem remota acima de 8 MB', 413, 'REMOTE_IMAGE_TOO_LARGE');
  const bytes = new Uint8Array(await response.arrayBuffer());
  if (!bytes.length || bytes.length > MAX_REMOTE_BYTES) {
    return fail('Imagem remota vazia ou acima de 8 MB', 413, 'REMOTE_IMAGE_TOO_LARGE');
  }
  if (!isAllowedImage(bytes, mime)) {
    return fail('O conteúdo baixado não corresponde a uma imagem válida', 415, 'INVALID_IMAGE_CONTENT');
  }
...
async function uploadBrandLogo(req: Request, env: Env) {
  ...
  if (mime !== 'image/webp') {
    return fail('A imagem final da marca precisa estar convertida para WEBP', 415, 'BRAND_IMAGE_NOT_WEBP');
  }
  ...
  const storageResponse = await fetch(
    `${env.SUPABASE_URL}/storage/v1/object/${encodeURIComponent(BRAND_BUCKET)}/${encodedObjectPath(objectPath)}`,
    { method: 'POST', headers: storageHeaders, body: bytes },
  );
  const storageText = await storageResponse.text();
  if (!storageResponse.ok) {
    throw new Error(`Supabase Storage ${storageResponse.status}: ${storageText.slice(0, 400)}`);
  }
  ...
  await adminFetch(
    env,
    `/rest/v1/brands?id=eq.${encodeURIComponent(brand.id)}&company_id=eq.${encodeURIComponent(COMPANY_ID)}`,
    {
      method: 'PATCH',
  ...
  const existing = await adminFetch(
    env,
    `/rest/v1/media_assets?company_id=eq.${encodeURIComponent(COMPANY_ID)}&owner_type=eq.brand&owner_id=eq.${encodeURIComponent(brand.id)}&kind=eq.brand-logo&select=id&order=created_at.desc&limit=1`,
  );
...
export async function handleBrandImagesRoute(req: Request, env: Env, path: string): Promise<Response | null> {
  try {
    if (path === '/api/admin/brand-images/search' && req.method === 'GET') return handleSearch(req, env);
    if (path === '/api/admin/brand-images/fetch' && req.method === 'GET') return fetchRemoteImage(req, env);
    if (path === '/api/admin/brand-images/upload' && req.method === 'POST') return uploadBrandLogo(req, env);
    return null;
  } catch (error) {
    console.error('Falha no fluxo de imagens de marcas', error);
    return fail(
      error instanceof Error ? error.message : 'Falha interna ao processar imagem da marca',
      500,
      'BRAND_IMAGE_INTERNAL_ERROR',
    );
  }
}
