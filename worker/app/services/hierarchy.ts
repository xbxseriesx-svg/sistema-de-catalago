...
export type HierarchyLevel = 'departamento' | 'secao' | 'categoria';
...
async function select(env: Env, table: string, query: string) {
  const data = await adminFetch(env, `/rest/v1/${table}?${query}`);
...
async function mutate(
  ...
  await adminFetch(env, `/rest/v1/${table}${query ? `?${query}` : ''}`, {
    method,
...
async function allNodes(env: Env, companyId: string): Promise<HierarchyNodeRow[]> {
  return await select(
    env,
    'hierarchy_nodes',
...
function expectedParent(level: HierarchyLevel): HierarchyLevel | null {
  if (level === 'departamento') return null;
  if (level === 'secao') return 'departamento';
  return 'secao';
}

function nodeSlug(level: HierarchyLevel, name: string, parent: HierarchyNodeRow | null) {
...
async function createNode(req: Request, env: Env, companyId: string, user: UserContext) {
  ...
  const level = clean(input.level || input.type) as HierarchyLevel;
  if (!['departamento', 'secao', 'categoria'].includes(level)) {
    return fail('Nível inválido', 400, 'INVALID_LEVEL');
  }
  ...
  let parent: HierarchyNodeRow | null = null;

  if (parentLevel) {
    if (!parentId) {
      return fail(
        level === 'secao' ? 'Selecione o departamento superior' : 'Selecione a seção superior',
        400,
        'PARENT_REQUIRED',
      );
    }
    ...
    if (!parent || parent.type !== parentLevel) {
      return fail(
        `O item pai precisa ser ${parentLevel === 'departamento' ? 'um Departamento' : 'uma Seção'}`,
        400,
        'INVALID_PARENT',
      );
    }
  ...
  if (duplicateNode(nodes, level, parentId, name)) {
    return fail('Já existe um item com este nome neste nível', 409, 'DUPLICATE');
  }
...
async function updateDescendantSlugs(
  env: Env,
  companyId: string,
  nodes: HierarchyNodeRow[],
  rootId: string,
  ...
  const visit = async (parentId: string, parentSlug: string): Promise<void> => {
...
async function updateNode(
  ...
  if (duplicateNode(nodes, current.type, current.parent_id, name, id)) {
    return fail('Já existe um item com este nome neste nível', 409, 'DUPLICATE');
  }
  ...
  await audit(env, companyId, user.id, 'hierarchy.update', id, {
    level: current.type,
    name,
    previousName,
    slug: nextSlug,
...
async function countProductsUsing(env: Env, companyId: string, node: HierarchyNodeRow) {
  const column = node.type === 'departamento'
    ? 'departamento_id'
    : node.type === 'secao'
      ? 'secao_id'
      : 'categoria_id';
...
async function deleteNode(env: Env, companyId: string, user: UserContext, id: string) {
  ...
  const current = nodes.find((node) => node.id === id);
  if (!current) return fail('Item da hierarquia não encontrado', 404, 'NOT_FOUND');
  if (nodes.some((node) => node.parent_id === id)) {
    return fail('Não é possível excluir: existem itens abaixo deste nível', 409, 'IN_USE');
  }
  if (await countProductsUsing(env, companyId, current)) {
    return fail('Não é possível excluir: existem produtos vinculados', 409, 'IN_USE');
  }
  ...
  await audit(env, companyId, user.id, 'hierarchy.delete', id, {
    level: current.type,
    name: current.name,
  });
  return ok({ id });
...
export async function handleHierarchyRoute(
  ...
  if (path === '/api/admin/hierarchy' && req.method === 'POST') {
    return createNode(req, env, companyId, user);
  }
